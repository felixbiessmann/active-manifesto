...
The scope of the manifesto project limits the to be annotated texts to a feasible amount, 'just' the party manifestos. But one can imagine scenarios in which not all data can be fully annotated. One such scenario could be a tight deadline for an analysis right before elections: parties release their programs only shortly before the elections and often these texts are too long to be read by one single person before the elections \cite{merz2017}. In addition to relying on experts to interpret these programs, it can be helpful to use models trained on previously annotated data to assess the policies proposed in the party programs. This can help to get a better overview of the policies especially when new parties enter the political stage, see e.g. \cite{bronline}.
